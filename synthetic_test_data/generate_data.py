...
def _create_epub_book(identifier, title, author="Synthetic Data Generator", lang="en", custom_metadata=None, add_default_metadata=True):
    ...
    if custom_metadata: # For adding specific or overriding metadata
        for prefix, name, value, others_dict in custom_metadata: # Ensure 'others' is a dict
...
def create_pdf_text_single_column(filename="single_column.pdf"):
    ...
    styleH2 = styles['h2']
    story = []
    story.append(Paragraph("The Philosophy of Synthetic Documents", styleH1))
    story.append(Spacer(1, 0.2*inch))
    story.append(Paragraph("Chapter 1: The Nature of the Artificial", styleH2))
    ...
    """
    story.append(Paragraph(text_ch1.replace("\n", "<br/>"), styleN))
    story.append(Spacer(1, 0.2*inch))
    story.append(Paragraph("Chapter 2: Implications for Knowledge Systems", styleH2))
    story.append(Spacer(1, 0.1*inch))
    ...
    """
    story.append(Paragraph(text_ch2.replace("\n", "<br/>"), styleN))
...
if __name__ == "__main__":
    ...
    create_epub_kant_style_footnotes()
    create_epub_hegel_sol_style_footnotes()
    create_epub_dual_note_system()
